import { humanizeDuration, stringifyDate } from '../utils/utils';
import { getComments } from '../mock/comment.js';

function createGenresTemplate(genres) {
  return genres.map((it) => `<span class="film-details__genre">${it}</span>`)
    .join('');
}

function createCommentTemplate(comment) {
  const {id, text, pic, name, commentDate} = comment;
  /* html */
  return `
    <li class="film-details__comment">
      <span class="film-details__comment-emoji">
        <img src="./images/emoji/${pic}.png" width="55" height="55" alt="emoji-${pic}">
      </span>
      <div>
        <p class="film-details__comment-text">${text}</p>
        <p class="film-details__comment-info">
          <span class="film-details__comment-author">${name}</span>
          <span class="film-details__comment-day">${stringifyDate(commentDate, 'YYYY:MM:DD HH:MM')}</span>
...
function createCommentsTemplate(commentsIDs) {
            ...
            <div class="film-details__info">
              ...
              <table class="film-details__table">
                ...
                <tr class="film-details__row">
                  ...
                  <td class="film-details__cell">${stringifyDate(releaseDate)}</td>
                </tr>
                <tr class="film-details__row">
